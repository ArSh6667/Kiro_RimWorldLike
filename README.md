...
### 需要帮助？
```bash
update-Log\setup-dotnet.bat
```
自动安装 .NET SDK（运行游戏的必要条件）。

## 📁 项目结构

为了保持主目录简洁，更新文档和测试工具已移动到 `update-Log/` 文件夹：
- 📖 **更新文档**: 安装指南、故障排除、功能说明等
- 🔧 **测试工具**: 构建测试、诊断工具、修复脚本等
- ⚙️ **辅助工具**: 安装助手、项目信息等

查看详细说明：`update-Log\README-UpdateLog.md`
...
- **🎮 相机控制**: WASD键移动视角，鼠标滚轮缩放，多种快捷键
- **🖱️ 交互视图**: 可缩放、滚动、跟随人物的地图视图
- **⚡ 实时渲染**: 60+ FPS的流畅游戏体验
- **📊 性能监控**: FPS计数器和游戏时间显示

### 🎯 相机控制 (新增!)
- **🖥️ 纯全屏体验**: 仅显示地图，ESC键呼出菜单
- **WASD / 方向键** - 超快移动视角 (6倍速度，无卡顿)
- **鼠标滚轮** - 完美缩放视角 (0.1x - 5.0x，彻底无黑幕)
- **空格键** - 快速居中到人物
- **R键** - 重置缩放
- **F键** - 切换跟随模式
- **ESC键** - 呼出/关闭控制菜单

详细说明: `update-Log\CAMERA-CONTROLS.md` | `update-Log\PURE-FULLSCREEN-ESC-MENU.md` | `update-Log\ZOOM-FIX-SPEED-BOOST.md`
